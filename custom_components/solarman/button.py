...
async def async_setup_entry(_: HomeAssistant, config_entry: ConfigEntry[Coordinator], async_add_entities: AddEntitiesCallback) -> bool:
    ...
    async_add_entities([SolarmanRestart(config_entry.runtime_data)])
...
class SolarmanRestart(SolarmanEntity, ButtonEntity):
    ...
    async def async_press(self):
        await request(f"http://{self.coordinator.device.config.host}/{LOGGER_RESTART_URL}", auth = LOGGER_AUTH, data = LOGGER_RESTART_DATA, headers = {"Referer": f"http://{self.coordinator.device.config.host}/restart.html"})
